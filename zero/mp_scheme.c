  ...
  struct gkyl_array *cflrate, struct gkyl_array *rhs)
{
  ...
  for (int d=0; d<mp->num_up_dirs; ++d) {
    ...
    while (gkyl_range_iter_next(&iter)) {
      ...
      mp->equation->rotate_to_local_func(cg->tau1_cov[dir], cg->tau2_cov[dir], cg->norm_cov[dir],
        qr_l, qlocal_l);
      mp->equation->rotate_to_local_func(cg->tau1_cov[dir], cg->tau2_cov[dir], cg->norm_cov[dir],
        qr_r, qlocal_r);
      ...
      mp->equation->rotate_to_global_func(cg->tau1_con[dir], cg->tau2_con[dir], cg->norm_con[dir],
        amdq_local, amdq_p);
      mp->equation->rotate_to_global_func(cg->tau1_con[dir], cg->tau2_con[dir], cg->norm_con[dir],
        apdq_local, apdq_p);
    }
    ...
    while (gkyl_range_iter_next(&iter)) {
      ...
      mp->equation->rotate_to_global_func(cg->tau1_cov[dir], cg->tau2_cov[dir], cg->norm_cov[dir],
        deltaf_local, deltaf);
