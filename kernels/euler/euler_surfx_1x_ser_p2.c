...
{ 
  ...
  const double *rhoux_l = &fluid_l[3]; 
  const double *rhouy_l = &fluid_l[6]; 
  const double *rhouz_l = &fluid_l[9]; 
  const double *energy_l = &fluid_l[12]; 

  const double *rho_c = &fluid_c[0]; 
  const double *rhoux_c = &fluid_c[3]; 
  const double *rhouy_c = &fluid_c[6]; 
  const double *rhouz_c = &fluid_c[9]; 
  const double *energy_c = &fluid_c[12]; 

  const double *rho_r = &fluid_r[0]; 
  const double *rhoux_r = &fluid_r[3]; 
  const double *rhouy_r = &fluid_r[6]; 
  const double *rhouz_r = &fluid_r[9]; 
  const double *energy_r = &fluid_r[12]; 
  ...
  q_lr[4] = 1.58113883008419*energy_l[2]+1.224744871391589*energy_l[1]+0.7071067811865475*energy_l[0]; 
  ...
  q_cl[4] = 1.58113883008419*energy_c[2]-1.224744871391589*energy_c[1]+0.7071067811865475*energy_c[0]; 
  ...
  q_cr[4] = 1.58113883008419*energy_c[2]+1.224744871391589*energy_c[1]+0.7071067811865475*energy_c[0]; 
  ...
  q_rl[4] = 1.58113883008419*energy_r[2]-1.224744871391589*energy_r[1]+0.7071067811865475*energy_r[0]; 
  ...
  Ghat_energy_l = 0.25*pl_r*uxl_r+0.25*pc_l*uxl_r+0.3952847075210474*energy_l[2]*uxl_r+0.3952847075210474*energy_c[2]*uxl_r+0.3061862178478972*energy_l[1]*uxl_r-0.3061862178478972*energy_c[1]*uxl_r+0.1767766952966369*energy_l[0]*uxl_r+0.1767766952966369*energy_c[0]*uxl_r+0.25*pl_r*uxc_l+0.25*pc_l*uxc_l+0.3952847075210474*energy_l[2]*uxc_l+0.3952847075210474*energy_c[2]*uxc_l+0.3061862178478972*energy_l[1]*uxc_l-0.3061862178478972*energy_c[1]*uxc_l+0.1767766952966369*energy_l[0]*uxc_l+0.1767766952966369*energy_c[0]*uxc_l-0.5*apdq_l[4]+0.5*amdq_l[4]; 
  Ghat_rho_r = 0.3952847075210474*rho_r[2]*uxr_l+0.3952847075210474*rho_c[2]*uxr_l-0.3061862178478972*rho_r[1]*uxr_l+0.3061862178478972*rho_c[1]*uxr_l+0.1767766952966369*rho_r[0]*uxr_l+0.1767766952966369*rho_c[0]*uxr_l+0.3952847075210474*rho_r[2]*uxc_r+0.3952847075210474*rho_c[2]*uxc_r-0.3061862178478972*rho_r[1]*uxc_r+0.3061862178478972*rho_c[1]*uxc_r+0.1767766952966369*rho_r[0]*uxc_r+0.1767766952966369*rho_c[0]*uxc_r-0.5*apdq_r[0]+0.5*amdq_r[0]; 
  ...
  Ghat_energy_r = 0.25*pr_l*uxr_l+0.25*pc_r*uxr_l+0.3952847075210474*energy_r[2]*uxr_l+0.3952847075210474*energy_c[2]*uxr_l-0.3061862178478972*energy_r[1]*uxr_l+0.3061862178478972*energy_c[1]*uxr_l+0.1767766952966369*energy_r[0]*uxr_l+0.1767766952966369*energy_c[0]*uxr_l+0.25*pr_l*uxc_r+0.25*pc_r*uxc_r+0.3952847075210474*energy_r[2]*uxc_r+0.3952847075210474*energy_c[2]*uxc_r-0.3061862178478972*energy_r[1]*uxc_r+0.3061862178478972*energy_c[1]*uxc_r+0.1767766952966369*energy_r[0]*uxc_r+0.1767766952966369*energy_c[0]*uxc_r-0.5*apdq_r[4]+0.5*amdq_r[4]; 
